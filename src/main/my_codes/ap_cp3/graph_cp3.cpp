...
/**********WAY 2 (using adjacency  list)**********************/
  ...
  for (int i = 0; i < V; i++) {
    ...
    for (int j = 0; j < total_neighbors; j++) {
      scanf("%d %d", &id, &weight);
      AdjList[i].push_back(ii(id - 1, weight));    // some index adjustment(because our index is 0 for vertex no. 1)
    }
  }
...
/**********WAY 3 (using edge list)**********************/
  ...
  for (int i = 0; i < E; i++) {
    ...
    EdgeList.push(make_pair(-weight, ii(a, b))); // trick to reverse sort order(or use Greater<int> in declaration)
  }
