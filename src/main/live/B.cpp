...
void solve(){
    ...
    vi v(n);
    rep(n) cin>>v[i];
    char char_v='a'; // starting from here
    multimap<int, char> mp;
    string s="";
    for(auto x:v){
        if(x==0){ // use a new character
            mp.insert({0, char_v});
            s+=char_v;
            dout<<"inserting "<< char_v<<endl;
            char_v++;
        }else{ // find a value with x-1 and remove it
            auto p = mp.find(x-1);
            s+=p->second;
            dout<<"inserting "<< p->second<<endl;
            mp.insert({x, p->second});
            mp.erase(p);
        }
//        show_pair1d(mp);
    }
